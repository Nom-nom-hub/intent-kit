...
## About Intent Kit and Intent

**Intent Kit** is a comprehensive toolkit for implementing Intent-Driven Development (IDD) - a methodology that emphasizes capturing and executing user intent while measuring outcomes against original goals. The toolkit includes templates, scripts, and workflows that guide development teams through a structured approach to building software focused on user outcomes.
...
Add the new agent to the `AGENT_CONFIG` dictionary in `src/intent_kit/__init__.py`. This is the **single source of truth** for all agent metadata:
...
```python
ai_assistant: str = typer.Option(None, "--ai", help="AI assistant to use: claude, gemini, copilot, cursor-agent, qwen, opencode, codex, windsurf, kilocode, auggie, codebuddy, new-agent-cli, roo, or q"),
```
